...
def compute_loss(model, batch, objective):
    
    mel, ling, pros, spk, length, max_len = batch
    batch_size = mel.size(0)
    stop_tokens = torch.FloatTensor(batch_size, max_len).zero_().to(mel.device)

    for i in range(batch_size):
        cur_ppg_len = length[i]
...
class MaskedMSELoss(nn.Module):
    def __init__(self, frames_per_step = 4):
        super().__init__()
    ...
    def forward(self, mel_pred, mel_pred_postnet, mel_trg, lengths, 
        ...
        return mel_loss, mel_loss_pre, mel_loss_post,  stop_loss
