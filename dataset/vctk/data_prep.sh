...
# shellcheck disable=SC1091
. dataset/vctk/parse_options.sh || exit 1;
...
[ ! -e "${db_root}/VCTKCorpusFullContextLabel/lab/mono/${spk}" ] && \
    echo "${spk} does not exist." >&2 && exit 1;
...
find "${db_root}/VCTK-Corpus/wav48/${spk}" -follow -name "*.wav" | sort | while read -r wav; do
    id=$(basename "${wav}" | sed -e "s/\.[^\.]*$//g")
    lab=${db_root}/VCTKCorpusFullContextLabel/lab/mono/${spk}/${id}.lab
